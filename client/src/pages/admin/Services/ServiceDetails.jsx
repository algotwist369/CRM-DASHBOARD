...
const ServiceDetails = () => {
  ...
  const businessName = service.business?.name || service.businessName || service.business?.businessName || 'N/A';
  const availableDaysText = service.availableDays && service.availableDays.length
    ? service.availableDays.join(', ')
    : 'Not specified';
  const cancellationPolicy = service.cancellationPolicy || {
    allowed: true,
    hoursBeforeService: 24,
    cancellationFee: 0
  };
  const staffCommissionText = service.staffCommission
