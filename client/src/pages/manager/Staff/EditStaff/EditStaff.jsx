...
const EditStaff = () => {
  ...
  const fetchStaffData = async () => {
    try {
      ...
      if (result.success) {
        ...
        console.error('Staff data error:', result.error)
      }
    } catch (error) {
      console.error('Error fetching staff data:', error)
      toast.error('An unexpected error occurred while loading staff data')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await managerService.updateStaff(id, formData)
      
      if (result.success) {
        toast.success('Staff member updated successfully!')
        navigate(`/manager/staff/${id}`, {
          state: { message: 'Staff member updated successfully!' }
        })
      } else {
        setErrors({ general: result.error || 'Failed to update staff member. Please try again.' })
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-4">
            <Button
              variant="outline"
              size="sm"
              onClick={() => navigate(`/manager/staff/${id}`)}
            >
            ...
            <div className="flex justify-between mt-8">
              ...
              {currentStep < steps.length - 1 ? (
                ...
                <Button
                  ...
                  loading={isLoading}
                >
                  {isLoading ? 'Updating Staff Member...' : 'Update Staff Member'}
                </Button>
              )}
            </div>
          </form>
